...
{
    ros::init(argc, argv, "pose_control_node");//ros系统的初始化，最后一个参数为节点名称
    ros::NodeHandle nh;
    ...
    geometry_msgs::PoseStamped pose_xyz;    
    pose_xyz.pose.position.x = 0;
    pose_xyz.pose.position.y = 0;
    pose_xyz.pose.position.z = 5;
    
    mavros_msgs::AttitudeTarget pose_qua;
    ...
    for(int i = 100; ros::ok() && i > 0; --i){
        local_pos_pub.publish(pose_xyz);
        ros::spinOnce();
        rate.sleep();
    }
    ...
    while(ros::ok()){
        ...
        if( fly_flag==0 && current_state.mode != "OFFBOARD" &&
            ...
            if( fly_flag == 0 && !current_state.armed &&
                (ros::Time::now() - last_request > ros::Duration(5.0))){
        ...
        if(fly_flag ==1 && ros::Time::now()-last_request > ros::Duration(5.0)){
            pose_qua.orientation.w=cos(30.0*3.14/180.0);
            pose_qua.orientation.z=sin(30.0*3.14/180.0);
            pose_qua.thrust=0.71;
            last_request = ros::Time::now();
            fly_flag=2;
            ROS_INFO("2");
        }
        ...
        if(fly_flag ==2 && ros::Time::now()-last_request > ros::Duration(5.0)){
            pose_qua.orientation.w=cos(10.0*3.14/180.0);
            pose_qua.orientation.x=sin(10.0*3.14/180.0);
            pose_qua.thrust=0.71;
            last_request = ros::Time::now();
            fly_flag=3;
            ROS_INFO("3");
        ...
        if(fly_flag ==3 && current_state.mode != "AUTO.LAND" &&
            ros::Time::now()-last_request > ros::Duration(5.0)){
        ...
        if(fly_flag==0 || fly_flag==1){
            local_pos_pub.publish(pose_xyz);
            ROS_INFO("fly high");
        }
        else{
            thrust_pub.publish(pose_qua);
            ROS_INFO("pose control");
        }
        
        
       
        
        // local_posit_pub.publish(posti);
